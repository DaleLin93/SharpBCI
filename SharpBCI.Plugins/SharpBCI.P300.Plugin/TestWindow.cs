...
using Result = SharpBCI.Experiments.P300.P300Experiment.Result;
using static SharpBCI.Experiments.P300.P300Experiment.Result;
using static SharpBCI.Experiments.P300.P300Experiment.Result.Trial;
...
namespace SharpBCI.Experiments.P300
{
    ...
    internal class TestWindow : RenderForm, IDisposable
    {
        ...
        private readonly P300Experiment _experiment;
        ...
        public TestWindow(Session session)
        {
            ...
            _session = session;
            _experiment = (P300Experiment) session.Experiment;
            _markable = session.StreamerCollection.FindFirstOrDefault<IMarkable>();
            _result = new Result { Trials = new LinkedList<Trial>() };

            _stageProgram = _experiment.CreateStagedProgram(session);
            _stageProgram.StageChanged += StageProgram_StageChanged;

            /* Initialize blocks */
            _blocks = new Block[(int)_experiment.Config.Test.Layout];
            for (var i = 0; i < _blocks.Length; i++)
            {
                var block = new Block
                {
                    Target = _blocks.Length / 2 == i,
                    Random = _experiment.Config.Test.TargetRate
                    .CreateRandomBoolSequence((int)(DateTimeUtils.CurrentTimeTicks << 1 + i)),
                    Size = new RawVector2(_experiment.Config.Gui.BlockLayout.Width, _experiment.Config.Gui.BlockLayout.Height),
                    BorderWidth = (float)_experiment.Config.Gui.BlockBorder.Width
                };
            ...
            _blockNormalColor = _experiment.Config.Gui.BlockNormalColor.ToSdColor().ToSdx();
            _blockActivedColor = _experiment.Config.Gui.BlockActivedColor.ToSdColor().ToSdx();
        }
        ...
        private void InitializeDirectXResources()
        {
            ...
            _bitmap = _experiment.Config.Gui.UseBitmap ? Properties.Resources.Einstein.ToD2D1Bitmap(_renderTarget) : null;
        }
        ...
        private void UpdateBlocks()
        {
            ...
            var layoutSize = _experiment.Config.Test.Layout.GetLayoutSize();
            var position = _experiment.Config.Gui.BlockPosition;
            float winW = Width, winH = Height;
            float blockFrameW, blockFrameH;
            float offsetX = 0, offsetY = 0;

            var blockWidth = (int)(_experiment.Config.Gui.BlockLayout.Width * scaleFactor);
            var blockMarginH = (int)(_experiment.Config.Gui.BlockLayout.HMargin * scaleFactor);
            ...
            var blockHeight = (int)(_experiment.Config.Gui.BlockLayout.Height * scaleFactor);
            var blockMarginV = (int)(_experiment.Config.Gui.BlockLayout.VMargin * scaleFactor);
            ...
            if (stage.Marker != null)
            {
                ...
                switch (e.Stage.Marker)
                {
                    ...
                    case MarkerDefinitions.TrialStartMarker:
                    {
                        ...
                        trial.SubTrials = new List<Trial.SubTrial>((int)(_experiment.Config.Test.SubTrialCount + 1));
                    ...
                    case MarkerDefinitions.TrialEndMarker:
                        _trial = null;
                        break;
                    case P300Experiment.SubTrialMarker:
                        ...
                        if (targetActived)
                            _markable?.Mark(P300Experiment.OddBallEventMarker);
                        _trial.SubTrials.Add(new SubTrial { Timestamp = CurrentTime, Flags = flags });
                        break;
                }
            }
