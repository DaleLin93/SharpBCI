...
namespace SharpBCI.Experiments.MI
{
    ...
    internal partial class TestWindow
    {
        ...
        [NotNull] private readonly MiExperiment _experiment;
        ...
        public TestWindow(Session session)
        {
            InitializeComponent();
            _session = session;
            _experiment = (MiExperiment) session.Experiment;
            _markable = session.StreamerCollection.FindFirstOrDefault<IMarkable>();

            if (MiExperiment.MiStimClientProperty.TryGet(session, out _miStimClient))
            {
            ...
            {
                _gazeFocusDetector = new GazeFocusDetector(session.Clock, (uint)TimeUnit.Millisecond.ConvertTo(_experiment.Config.Test.GazeToFocusDuration, session.Clock.Unit));
            ...
            Background = new SolidColorBrush(_experiment.Config.Gui.BackgroundColor.ToSwmColor());
            CueText.Foreground = new SolidColorBrush(_experiment.Config.Gui.FontColor.ToSwmColor());
            CueText.FontSize = _experiment.Config.Gui.FontSize;
            ProgressBar.Background = new SolidColorBrush(_experiment.Config.Gui.ProgressBarColor[ColorKeys.Background].ToSwmColor());
            ProgressBar.Foreground = new SolidColorBrush(_experiment.Config.Gui.ProgressBarColor[ColorKeys.Foreground].ToSwmColor());
            ProgressBar.BorderThickness = new Thickness(_experiment.Config.Gui.ProgressBarBorder.Width);
            ProgressBar.BorderBrush = new SolidColorBrush(_experiment.Config.Gui.ProgressBarBorder.Color.ToSwmColor());
        ...
        private void StageProgram_StageChanged(object sender, StageChangedEventArgs e)
        {
            ...
            var repeat = _experiment.Config.Test.Repeat;
            var forceReset = _experiment.Config.Test.ForceReset;
